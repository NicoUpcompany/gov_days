...
function getQuestions(req, res) {
    Question.find({ active: { $ne: false } }).populate('user', 'name lastname email phone enterprise position sector').sort({ order: "asc" }).exec((err, questionStored) => {
        if (err) {
            res.status(500).send({ok: false, message: "Error del servidor"});
        } else {
            if (!questionStored) {
...
function getUserQuestion(req, res) {
    ...
    Question.findById({ user: id }).populate('user', 'name lastname email phone enterprise position sector').sort({ order: "asc" }).exec((err, questionStored) => {
        if (err) {
            res.status(500).send({ok: false, message: "Error del servidor"});
        } else {
            if (!questionStored) {
